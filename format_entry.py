...
def format_entry(row, include_citation=True):
    ...
    pub_link_badge = f"[:green-badge[Publication link]]({link_to_publication})" if link_to_publication else ''
    zotero_link_badge = f"[:gray-badge[Zotero link]]({zotero_link})" if zotero_link else ''
